...
**After the existing state declarations**, add your two new variables:
...
**Why `[]` (empty array)?**
- `selectedStatuses` starts empty because no statuses are selected yet
- Empty array means "no status filtering active"

**Why `CourseStatus[]` and not `string[]`?**
...
Write these two state declarations in your code at the correct location.
...
We need to search across **multiple fields**:

```
CourseRow
  ├─ course.courseTitle      ← search here
  ├─ course.levelName        ← search here
  ├─ course.trainingTypeName ← search here
  └─ program.programName     ← search here
```
...
- **Performance**: We're about to check 4 fields. If we call `.toLowerCase()` 4 times, we're doing the same work 4 times
...
**Breaking this down**:
- `.toLowerCase()` converts the field to lowercase
...
const matchesLevel = row.course.levelName.toLowerCase().includes(searchLower);
const matchesType = row.course.trainingTypeName.toLowerCase().includes(searchLower);
const matchesProgram = row.program.programName.toLowerCase().includes(searchLower);

return matchesTitle || matchesLevel || matchesType || matchesProgram;
```
...
Notice: We're always calling `.filter()`, not using a ternary. The logic for "show all when filter is empty" is **inside** the filter checks.

### 5.2 Building Each Check

#### Check 1: matchesProgram

**Logic**:
- If no programs are selected, show all courses → match
- If programs are selected, show only those programs → check if row's program is in the list
...
Think of it as: "Does this course pass the program check?"
- If no program filter is active, answer is always "yes" (true)
- If a program filter is active, answer is "yes" only if it's in the list

#### Check 2: matchesStatus

**Same pattern as matchesProgram**:
```typescript
const matchesStatus = selectedStatuses.length === 0 || selectedStatuses.includes(row.status);
```

**Logic**:
- If no statuses are selected, show all → match
...
**Simple!** We already did the complex logic in the helper function.
...
2. Calculate three boolean values
3. Return `true` only if all three are `true`
4. Keep only courses that returned `true`

**Short-circuit evaluation bonus**:
If `matchesProgram` is `false`, JavaScript doesn't even check the other two. It knows the result is `false` already. This is a small performance optimization.
...
const filteredCourses = flatCourses.filter((row) => {
  ...
  console.log("Filters active:", { searchText, selectedPrograms, selectedStatuses });
  console.log("Row checks:", { matchesProgram, matchesStatus, matchesSearchText });
